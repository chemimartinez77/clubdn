...
export default function ShareDualGameModal({ isOpen, onClose, primaryGame }: ShareDualGameModalProps) {
  ...
  useEffect(() => {
    if (game2) {
      setTitle(`${primaryGame.name} o ${game2.name}`);
    } else {
      setTitle(primaryGame.name);
    }
  }, [game2, primaryGame.name]);

  const handleShare = () => {
    if (!game2) return;
    const apiBase = import.meta.env.VITE_API_URL || 'http://localhost:5000';
    const previewUrl = `${apiBase}/preview/dual?bgg1=${encodeURIComponent(primaryGame.id)}&bgg2=${encodeURIComponent(game2.id)}&title=${encodeURIComponent(title)}`;
    ...
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Proponer 2 juegos" size="sm">
        <div className="p-6">
          ...
          {game2 && (
            <div className="mb-4">
              <p className="text-xs text-[var(--color-textMuted)] mb-1">Título del mensaje</p>
              <input
                type="text"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-[var(--color-cardBorder)] bg-[var(--color-bg)] text-sm text-[var(--color-text)] focus:outline-none focus:border-[var(--color-primary)]"
              />
            </div>
          )}
