...
export default function MembershipManagement() {
  ...
  const [bajasMonth, setBajasMonth] = useState(prevMonthDate.getMonth()); // 0-indexed, mes natural anterior
  const [bajasYear, setBajasYear] = useState(currentYear);
  ...
  const bajasQueryParams = bajasMode === 'days'
    ? `days=${bajasDays}`
    : `month=${bajasMonth + 1}&year=${bajasYear}`;

  const { data: bajasResponse, isLoading: bajasLoading } = useQuery<RecentBajasResponse>({
    queryKey: ['bajas-recientes', bajasMode, bajasMode === 'days' ? bajasDays : bajasMonth, bajasMode === 'month' ? bajasYear : null],
    queryFn: async () => {
      ...
      <div className="max-w-full mx-auto space-y-6 px-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 pt-4 border-t border-[var(--color-cardBorder)] text-sm text-[var(--color-textSecondary)]">
              {isLoading ? null : (
                <span>
                  {filteredUsers.length} {filteredUsers.length === 1 ? 'usuario' : 'usuarios'} en el listado
                  {filteredUsers.length !== (response?.users?.length ?? 0) && (
                    <span className="ml-1">(de {response?.users?.length ?? 0} en total)</span>
                  )}
                </span>
              )}
            </div>
