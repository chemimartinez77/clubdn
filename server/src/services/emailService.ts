...
/**
 * Email a admins cuando un miembro EN_PRUEBAS es promovido automáticamente a COLABORADOR
 */
export const sendMemberPromotedEmail = async (
  adminEmail: string,
  adminName: string,
  memberName: string,
  memberEmail: string,
  ...
  const html = `
    ...
    <html>
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
          <h2 style="color: #059669; margin-top: 0;">Miembro promovido automáticamente</h2>
          <p>Hola ${adminName},</p>
          <p>El siguiente miembro ha completado su período de prueba de <strong>60 días</strong> y ha sido promovido automáticamente a <strong>COLABORADOR</strong>:</p>
          <div style="background: white; padding: 20px; border-radius: 8px; border-left: 4px solid #059669; margin: 20px 0;">
            <p style="margin: 8px 0;"><strong>👤 Nombre:</strong> ${memberName}</p>
            <p style="margin: 8px 0;"><strong>📧 Email:</strong> ${memberEmail}</p>
          </div>
          <p>Puedes revisar y gestionar este miembro desde el directorio:</p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="${membersUrl}" style="background-color: #059669; color: white; padding: 14px 30px; text-decoration: none; border-radius: 5px; display: inline-block; font-weight: bold; box-shadow: 0 4px 6px rgba(5, 150, 105, 0.3);">
              Ver directorio de miembros
            </a>
          </div>
          <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 30px 0;">
          <p style="color: #9ca3af; font-size: 12px; margin-bottom: 0;">
  ...
  return sendEmail({
    to: adminEmail,
    subject: `Nuevo colaborador: ${memberName} ha completado su período de prueba`,
    html,
    template: 'member_promoted',
  });
};
